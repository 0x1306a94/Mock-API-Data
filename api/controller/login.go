...
func Login(c *gin.Context) {
	...
	if err != nil {
		if gorm.IsRecordNotFoundError(err) {
			c.JSON(http.StatusBadRequest, util.GenerateErrorResponse(400, "没有该用户"))
		} else {
			c.JSON(http.StatusBadRequest, util.GenerateErrorResponse(400, err.Error()))
		}
		return
	...
	if user.Id == 0 {
		c.JSON(http.StatusBadRequest, util.GenerateErrorResponse(400, "用户名或者邮箱错误"))
		return
	}

	if !util.ValidationPassword(login.Password, user.Password) {
		c.JSON(http.StatusBadRequest, util.GenerateErrorResponse(400, "密码错误"))
		return
	}
